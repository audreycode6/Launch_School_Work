...
class TTTGame:
    ...
    def critical_square(self, row, player):
        if self.board.count_markers_for(player, row) == 2:
            for mark in row:
                if self.board.is_unused_square(mark):
                    return mark
        return None

    def find_critical_square(self, player):
        for row in TTTGame.WINNING_COMBINATIONS:
    ...
    @staticmethod
    def _join_or(choices_list, separator=', ', end='or'):
        ...
        all_but_last = separator.join(choices_list[:length-1])
        return f"{all_but_last}{separator}{end} {last_choice}"

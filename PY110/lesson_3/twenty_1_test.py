...
# TODO:
''' 
 - cards written out like this are a bit hard to follow, especially if we end up with more than 3 or 4 cards.
         It would be really cool if we had some actual cards to look at
        +------+
        |♠     |
        |  10  |
        |     ♠|
...
def display_intro():
    ...
    display_prompt("You always go first, and can decide to either hit or stay.\n"
...
def game_results(winner, player_sum, dealer_sum, both_player_cards):
    ...
    display_prompt(f'Your score: {player_sum} | Your cards: {format_cards(both_player_cards[0])}')
    display_prompt(f'Dealer score: {dealer_sum} | Dealer cards: {format_cards(both_player_cards[1])}\n')
